 ...
 * @author tadij
 */
public class CommandKvar implements CommandChain {
    ...
    public void execute(String command) {
        if (command.contains("KVAR")) {
            commandParts = command.split(";|,");
            for (Vehicle currentVehicle : Vehicle.allVehicles) {
                for (int i = 0; i < commandParts.length; i++) {
                    if (currentVehicle.getId().equals(commandParts[i])) {
                        currentVehicle.setKvar();
                        currentVehicle.setVehicleState(currentVehicle.getInoperative());
                    }
                }
            }
        } else {
